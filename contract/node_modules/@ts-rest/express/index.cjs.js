...
const validateRequest = (req, res, schema, options) => {
    const paramsResult = core.checkZodSchema(req.params, schema.pathParams, {
        passThroughExtraKeys: true,
    });
    const headersResult = core.checkZodSchema(req.headers, schema.headers, {
        passThroughExtraKeys: true,
    });
    const query = options.jsonQuery
        ? core.parseJsonQueryObject(req.query)
        : req.query;
    const queryResult = core.checkZodSchema(query, schema.query);
    const bodyResult = core.checkZodSchema(req.body, 'body' in schema ? schema.body : null);
...
const initializeExpressRoute = ({ implementationOrOptions, schema, app, options, }) => {
    ...
    const mainReqHandler = async (req, res, next) => {
        try {
            ...
            if (result.body instanceof stream.Stream) {
